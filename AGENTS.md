...
- 双变体发布：同一源码打两个包——stable `Charging_Record.zip` 与实验 `Charging_Record_ML.zip`。CI 打 ML 包时改 `name=ChargingRecord Evolution ML`、description 加 `[ML实验版]` 前缀并**删除 updateJson 行**，故 ML 仅手动刷入、无应用内更新；`updateJson` 永远只指向 stable 资产。
