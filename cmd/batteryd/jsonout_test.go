...
func TestRenderJSONGoldenWithNulls(t *testing.T) {
	...
	want := `{"channel":"stable","design_mah":4000,"full_mah":3850,"cycles":210,"pct":96,"est_mah":null,"est_mah_sigma":null,"samples":0,"cycle_equiv":0,"r_moh":null,"temp_c":null,"trend_mah_per_week":null,"updated":"2026-08-26 14:30:05","recent":[],"sessions":null,"rest_points":null,"ccct":null,"samples_n":0}`
	got, err := RenderJSON("stable", d, snap, []TsVal{}, nil, nil, nil, 0, now)
	if err != nil {
...
func TestRenderJSONGoldenPartialNulls(t *testing.T) {
	...
	now := time.Date(2026, 8, 26, 14, 30, 5, 0, time.UTC)
	want := `{"channel":"stable","design_mah":null,"full_mah":3850,"cycles":null,"pct":null,"est_mah":null,"est_mah_sigma":null,"samples":0,"cycle_equiv":0,"r_moh":null,"temp_c":null,"trend_mah_per_week":null,"updated":"2026-08-26 14:30:05","recent":[],"sessions":null,"rest_points":null,"ccct":null,"samples_n":0}`
	got, err := RenderJSON("stable", d, Snapshot{}, []TsVal{}, nil, nil, nil, 0, now)
	if err != nil {
		t.Fatalf("RenderJSON: %v", err)
	}
	if string(got) != want {
...
func TestRenderJSONSanitizesNonFiniteFloats(t *testing.T) {
	...
	out, err := RenderJSON("stable", d, snapNaN, nil, nil, nil, nil, 0, now)
	if err != nil {
		t.Fatalf("RenderJSON: %v", err)
	}
	if len(out) == 0 {
	...
	out, err = RenderJSON("stable", d, snapInf, nil, nil, nil, nil, 0, now)
	if err != nil {
		t.Fatalf("RenderJSON: %v", err)
	}
	if len(out) == 0 {
...
func TestRenderJSONNewFields(t *testing.T) {
	...
	b, err := RenderJSON("stable", d, snap, recent, sess, rests, nil, 42, time.Unix(0, 0))
	if err != nil {
		t.Fatal(err)
	}
	s := string(b)
	...
	b2, _ := RenderJSON("stable", Design{}, Snapshot{}, nil, []sessionEntry{}, []restEntry{}, []ccctEntry{}, 0, time.Unix(0, 0))
	if strings.Contains(string(b2), `"sessions":[{`) {
		t.Fatalf("空数据不应出会话数组元素: %s", b2)
	}
	for _, want := range []string{`"sessions":[]`, `"rest_points":[]`, `"ccct":[]`} {
...
func TestRenderJSONCcctEntries(t *testing.T) {
	...
	b, err := RenderJSON("stable", Design{}, Snapshot{}, nil, nil, nil, ccct, 0, time.Unix(0, 0))
	if err != nil {
		t.Fatal(err)
	}
