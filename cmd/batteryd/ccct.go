...
// 于该窗内充入的电量 Q=I·t。窗口取 0.1 V 格点（Lin 2022 原文口径）；窗位为本机
// 数据自定标——两台实测设备全会话扫描定在 4.2~4.3V（论文最优起点 3.77V 属其
// 电池化学体系，不直接迁移），3.9~4.0V 落在低电量区，日常补电会话起点普遍高于
// 窗上沿（belowLo 永不成立）。倍率门控（≤1C）由调用方持 designUA 过滤，本文件
// 保持纯函数。
