...
另外模块还在顺手收集几样东西，存在本地等数据攒够了做下一步：静置电压（拔掉负载后的恢复电压，要熄屏静置约 10 分钟才收敛）、恒流时长（恒流段里充过 4.2~4.3 V 这 0.1 V 窗口的耗时，随老化变短；LFP 电池满充只有 3.65 V，永远到不了这个窗口，该特征会一直显示采集中，不是故障）、增量容量主峰（在倍率不超过 1C 的采信会话上计算，倍率越高峰越不锐利）。这几个特征目前只看趋势方向，不承诺绝对精度。
